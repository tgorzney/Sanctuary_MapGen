...
Economy: passive production alloys 5/s, energy 50/s. Template `initial` alloys 250, energy 2500; `storage` cap alloys 500, energy 5000 (own buffer, separate from built storage structures).
**Open question (2026-08-16), do not state either number as settled:** it is NOT confirmed which starting bank actually applies in a real match. Traced `CreateArmies()`/`CreateArmy()` in `common/gameUtils.lua` end to end — it only passes `(armyID, mapStartSlotName, faction, civilian)`; no Alloys/Energy field is threaded through anywhere in visible Lua, and `CreateArmy` itself is native (not defined in any `.lua` file found). Separately, `common/systems/templateLoader.lua` never reads `economy.initial` at all — only `economy.buildTime`/`economy.cost` are processed there. So both candidate values are plausible but unverified from static tracing alone: the `.sanmap`'s per-army `Alloys`/`Energy` (e.g. Pandemonium Isthmus sets 100/1000) AND this template's `initial` (250/2500) each *could* be native-only fields that do apply, or dead data that doesn't. **Resolve by launching a skirmish on the map and reading the starting-resource HUD** — faster and more certain than more static tracing. Until then, treat the true starting bank as unknown between these two candidates.
