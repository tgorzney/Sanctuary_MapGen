...
**partial** layer subset (also confirmed working live in-game, 2026-08-29). This ratification
promotes that technique to first-class knowledge-pack law, following the same process used for the
Map Scenario system (§15): full technical contract in `NAVMAP_MODIFIER_BLOCKER_SPEC.md`, not
re-derived here.

**Status:** both techniques are **confirmed shipped** — the all-layer technique (§22.3, confirmed
twice) and the partial-layer technique (§22.4, confirmed once, 2026-08-29, human confirmation: "sea
blocker working") — the same evidentiary bar. Neither introduces a new SanGen `PARAMS`/`IO`/`UI`
construct today (§22.9); this ratification is knowledge-pack law about a hand-authored Lua pattern,
the same status `MAP_UNIT_SPAWNING_SPEC.md`'s generator-function patterns already hold.
