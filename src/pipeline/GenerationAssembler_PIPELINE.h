...
// Noise/Blend -> Mask -> Erosion -> Thermal -> Flow/Accumulation -> Placement -> Bake.
...
// and reads the settings from the caller's MapRecipe.
#pragma once
#include <cstddef>
#include <functional>
#include <string>
...
public:
    ...
    // Per-stratum mask settings the Mask stage consumes; sized to MapFields::stratumCount.
    std::vector<Params::StratumMask>& StratumMaskSettings() { return stratumMaskSettings; }
...
private:
    ...
    std::vector<Params::StratumMask> stratumMaskSettings;
