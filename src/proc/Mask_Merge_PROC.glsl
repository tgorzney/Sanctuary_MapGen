...
// ImportedMaskMode merges and the per-stratum output remap. Same expressions in the same order
// as the CPU header (Constitution §4). The bilinear resampler stays in Mask_PROC.glsl because
// it reads the stored-art buffer, which cannot cross a GLSL compilation-unit boundary.
...
// zero-reciprocal sentinel used elsewhere.
float remapMaskValue(float value, float remapMinimum, float inverseRemapRange,
                     float maskMinimum, float maskMaximum) {
    if (inverseRemapRange <= 0.0) return value >= remapMinimum ? maskMaximum : maskMinimum;
    return clamp((value - remapMinimum) * inverseRemapRange, maskMinimum, maskMaximum);
}
