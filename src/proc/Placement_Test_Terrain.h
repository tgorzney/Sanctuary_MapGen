...
// `worldUnitsPerGenerationCell` — the same `Params::Geometry` value the recipe carries (ARCH §7.1, M5-0d),
// so a scaled fixture and the Placement stage reading it cannot disagree about cell world-size.
inline void BakeSlopeField(Data::MapFields& fields, float worldUnitsPerGenerationCell = 1.0f) {
    const Proc::MaskConstants constants;
    const float cellWorldSize = worldUnitsPerGenerationCell > 0.0f ? worldUnitsPerGenerationCell : 1.0f;
...
inline void BuildTestFields(Data::MapFields& fields, float worldUnitsPerGenerationCell = 1.0f) {
    ...
    BakeSlopeField(fields, worldUnitsPerGenerationCell);   // Mask's other output, which Placement READS (M5-0c)
}
...
// `worldUnitsPerGenerationCell` the stage multiplied by (Placement_Accept_PROC does the identical
...
inline bool AllWithinGates(const Data::PlacementInstances& instances, const Data::MapFields& fields,
                           ...
                           float worldUnitsPerGenerationCell = 1.0f) {
    const float tangent = std::tan(maxSlopeDegrees * 3.14159265f / 180.0f);
    const float gradientLimitSquared = tangent * tangent;
    const float cellReciprocal = 1.0f / (worldUnitsPerGenerationCell > 0.0f ? worldUnitsPerGenerationCell : 1.0f);
