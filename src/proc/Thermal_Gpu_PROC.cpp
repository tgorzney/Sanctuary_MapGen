...
std::string BuildShaderDefinitions() {
    return ShaderDefinition("THERMAL_TILE_WIDTH",  Sys::WorkgroupSize::kFieldTileWidth)
         ...
         + ShaderDefinition("THERMAL_SLOT_MASK_EPSILON", ThermalConstantSlot::maskWeightEpsilon)
...
void ThermalStage::UploadFieldsToGpu() {
    ...
    const std::size_t maskBytes = heightBytes * Data::MapFields::stratumCount;
    ...
    gpuResourceManager->EnsureBuffer("thermalMaskA", maskBytes);
    gpuResourceManager->EnsureBuffer("thermalMaskB", maskBytes);
    ...
    for (int stratum = 0; stratum < Data::MapFields::stratumCount; ++stratum) {
        const float* source = mapFields.materialMasks[stratum].Data();
    ...
    gpuResourceManager->UploadBuffer("thermalMaskA", gpuTransferBuffer.data(), maskBytes);
...
void ThermalStage::RunGpuSweeps(std::string& heightReadName, std::string& maskReadName) {
    ...
    maskReadName = "thermalMaskA";
    std::string heightWriteName = "thermalHeightB";
    std::string maskWriteName = "thermalMaskB";
    gpuResourceManager->SetUniformInt(program, "vertexSize", vertexSize);
    gpuResourceManager->SetUniformInt(program, "transportMaterialMasks", bTransport ? 1 : 0);
    ...
    for (int iteration = 0; iteration < constants.iterationCount; ++iteration) {
        ...
        gpuResourceManager->BindBuffer(maskReadName, 2);
        gpuResourceManager->BindBuffer(maskWriteName, 3);
        ...
        if (bTransport) maskReadName.swap(maskWriteName);
        ++completedIterationCount;
    }
}

void ThermalStage::ReadbackFieldsFromGpu(const std::string& heightReadName, const std::string& maskReadName) {
    const std::size_t cellCount = mapFields.heightfield.CellCount();
    const std::size_t heightBytes = cellCount * sizeof(float);
    ...
    gpuResourceManager->ReadbackBuffer(maskReadName, gpuTransferBuffer.data(),
...
void ThermalStage::RunOnGpu() {
    ...
    std::string maskReadName;
    RunGpuSweeps(heightReadName, maskReadName);
    ReadbackFieldsFromGpu(heightReadName, maskReadName);
