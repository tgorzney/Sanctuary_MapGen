...
// kinds (Alloy/Plasma/Spawn). Layer: PARAMS. Settings only. ARCH.md §11 (completes
// SANMAP_FORMAT_SPEC Correction 7): map-scoped, not per-rule — the same global-vs-per-rule scope
// split as `Symmetry`/`SlopeDefaults` vs. their per-rule overrides. `Plasma` = Energy, a real
// planned resource type (ARCH §11), not the v1 invention IO_PARITY_REPORT.md Decision #5 flagged.
#pragma once
#include <string>
