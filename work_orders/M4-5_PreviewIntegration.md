...
## Root problem
`UI_FRAMEWORK_SPEC` / ARCH §6.1: a cheap visual tweak must not trigger a full regen. Two
...
- `src/pipeline/GenerationAssembler_PIPELINE.*` (or a small `PreviewDriver_PIPELINE.*`) —
  ...
  call `Data::SpatialGrid::Build(...)` immediately after the Placement stage (ARCH §8.3:
