...
## Backend policy
N/A — a handful of `ifstream` reads plus one `Sys::CheckLuaSyntax` compile in the test, at most once
per export attempt. No compute dispatch, no SIMD, no GPU handle.
...
6. **Real bundled resource self-check**, using the CMake-staged `SANGEN_V2_LUA_RESOURCE_DIRECTORY`
   (argv[2]): read `SanGenScenarioRuntime.lua` from the real staged path, then assert (a) first line
   == `Io::kScenarioGeneratedFileBannerLine` exactly (`#include "ScenarioScript_DataLua_IO.h"` in the
   test only — proves the two literals stay in lockstep); (b) contains `"Scenario = {}"` and **not**
   `"local Scenario"`; (c) contains `"function Scenario.ResolveAndApply"` and
   `"function Scenario.SpawnNavalFleets"`; (d) `Sys::CheckLuaSyntax(runtimeLuaText).bSucceeded ==
   true`.
