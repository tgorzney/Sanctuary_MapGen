#include "Gen_FlowAndAccumulation.h"
...
void Gen_FlowAndAccumulation::Process(const FloatMask& outMap, const GenerationParams& params, GenerationResult& inOutResult, size_t currentErosionHash, size_t& outFlowHash) {
                                ...
                                for (int x = tileX; x < tileX + tileSize && x < vertSize - 1; x += 8) {
                                    ...
                                    for(int dy = -1; dy <= 1; ++dy) {
                                        for(int dx = -1; dx <= 1; ++dx) {
                                            ...
                                            __m256 incomingVelVec = _mm256_loadu_ps(&flowPtr[y * vertSize + x]);
